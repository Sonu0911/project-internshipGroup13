...
const createIntern = async function(req, res) {
    try {
        ...
        if (Object.keys(data) != 0) {
            ...
            if (!(/^[6-9]\d{9}$/.test(mobile))) { return res.status(400).send({ Status: false, ERROR: "Please provide the mobile field and enter a valid 10 digit mobile number" }) }
            ...
            let internCreated = await internsModel.create(data)
...
const getDetails = async function(req, res) {
    ...
    try {
        ...
        let availableInterns = await internsModel.find({ collegeId: requestedCollege._id }).select({ _id: 1, name: 1, email: 1, mobile: 1 })
        ...
        if (availableInterns.length > 0) {
            result["Interns"] = availableInterns
            return res.status(200).send({ Data: result })
        }
        if (availableInterns.length == 0) {
            result["Interns"] = "No interns are available for the respected college."
            return res.status(200).send({ Data: result })
        }
